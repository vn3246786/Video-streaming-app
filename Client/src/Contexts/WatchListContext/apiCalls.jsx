...
dispatch({type:'start'})
try {
    const res =await axios.get(`${import.meta.env.VITE_API_URL}/api/users/watchlist/${id}`,{headers:{
...
dispatch({type:'start'})
try {
    const res =  await axios.put(`${import.meta.env.VITE_API_URL}/api/users/watchlist/${id}`,NewWatchList,{headers:{
        token:'bearer '+accesstoken
    }})
    if(findError(res.data)){
        dispatch({type:"failure" ,payload:res.data})
