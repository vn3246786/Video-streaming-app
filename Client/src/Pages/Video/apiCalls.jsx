...
try {
    await axios.put(`/api/users/update-recommendations`,{recommendations:recommendations},{headers:{
...
export async function updateWatchHistory (update,id,timestamp,dispatch,accesstoken){
    ...
    dispatch({type:"start"})
try {
   const res= await axios.put(`/api/users/update-watchhistory${update?"?update=true":""}`,{id:id,timestamp:timestamp},{headers:{
        token:"bearer "+accesstoken
    }})
